...
**Premier League player identity: two write paths, reconciled by name, not merged automatically for free.** The line above — "Premier League gets full squads from football-data.org and the complete depth/statistics set from the FPL API on top" — describes the *intended* result, not something that happens by construction. football-data.org and the FPL API each independently create a `players` row for the same human being: football-data via `fd_id` (full squad + club, no photo), FPL via `fpl_id` (deep stats + photo, no club of its own). A plain `onConflict: 'fpl_id'` upsert can never land the FPL row onto the football-data row, because Postgres treats `NULL` as distinct from every other `NULL` in a unique index, and the football-data row's `fpl_id` starts out `NULL`. Until this was fixed, every Premier League player existed as two disjoint rows — confirmed live: 2,448 `players` rows, only 1,881 with a `team_id`; the other 567 were the entire Premier League roster, floating with no club, no visible photo on a squad page (which queries by `team_id`), and a duplicate entry in name search (`players_name_trgm`).

The fix has two halves, neither of them fuzzy matching, an alias table, or manual overrides:

- **FPL's 20 Premier League clubs are matched onto this project's own `teams` rows** (`matchFplTeamsToClubs`, `lib/ingest/playerIdentity.ts`) by normalised name, with a short-code fallback (football-data's `tla` vs FPL's `shortName`) for the common case where FPL's club name is itself an informal nickname sharing nothing with football-data's full name (`"Spurs"` vs `"Tottenham Hotspur FC"`, `"Man Utd"` vs `"Manchester United FC"`). Verified against this project's own database: **19 of 20 clubs matched**; Nottingham Forest is the one honest miss — football-data's own code (`tla: "NOT"`) and FPL's own code (`short_name: "NFO"`) genuinely disagree, so this is a real gap between the two providers' data, not a defect in the matcher. A club that never resolves has no candidate pool at all, so every one of its FPL players is unmatchable by construction, not just unlucky.
- **Each FPL player is matched onto an existing football-data player** via a tiered, *club-scoped* matcher (`matchPlayersTiered`, `lib/ingest/playerMatch.ts`). A first pass used a single global normalised-full-name join and only reached 253 of ~620 Premier League FPL players, because FPL's `first_name + second_name` is the player's full **legal** name while football-data stores the **display** name a broadcast graphic would use — "David Raya Martín" (FPL) vs "David Raya" (football-data), "Gabriel dos Santos Magalhães" vs "Gabriel Magalhães". FPL also carries `web_name`, the short form the game itself displays (e.g. "Raya", "Gabriel"), which is hopelessly ambiguous across the whole league but nearly unique within one ~25-player club. The matcher therefore tries three tiers *within the FPL player's own club*, in order, stopping at the first tier that narrows the candidate pool to exactly one player: (1) exact normalised full name — the original signal, still tried first; (2) normalised `web_name` equal to the last token of a candidate's normalised name; (3) normalised `web_name` equal to any whole token of a candidate's normalised name (catches a shared first name like "Gabriel"). **A tier that yields two or more within-club candidates is treated as ambiguous and skipped, not guessed at** — if every tier is ambiguous or empty, the player is left unmatched, honestly.

**Final observed numbers** (both against the live database — see `.superpowers/sdd/final-fixes-b-report.md` for the full before/after and the residual list): starting from the 567-row bug state (2,448 `players` rows, 1,881 with a `team_id`), the exact-name pass plus the tiered repair together resolved the Premier League roster to **1,945 total players, 1,940 with a `team_id`, 313 rows carrying both `fd_id` and `fpl_id`, and 5 players still fully orphaned** (no club, no football-data row) — down from 124 orphans immediately before the tiered pass ran. The tiered pass alone contributed 60 of those merges (0 exact — the exhausted first pass had already claimed every exact match — 55 last-token, 5 whole-token).

**What an unmatched or duplicate-looking player looks like in Phase B, concretely — the residual to design for, not assume away:**

- **All 5 fully-orphaned players are Nottingham Forest signings** (e.g. Felipe Rodrigues da Silva, Igor Jesus Maciel da Cruz, Murillo Costa dos Santos) — a direct, mechanical consequence of the club-level miss above: with no resolved `team_id` for that club, these players have no candidate pool to match against and no club to fall back to either. This is not a name-matching failure; it is the one club-level gap propagating downstream.
- **A same-club-token ambiguity that stayed unmatched on purpose:** Arsenal's "Gabriel dos Santos Magalhães" (web_name "Gabriel") did not resolve even under tier 3, because Arsenal also fields "Gabriel Fernando de Jesus" (football-data display name "Gabriel Jesus") — both candidates' normalised names contain the token "gabriel", so the tier is genuinely ambiguous and the matcher correctly declined to guess between two real players rather than risk merging the wrong one.
- **A multi-token `web_name` that the tiers don't cover — the single largest reason a player stays unmatched.** Tiers 2–3 assume `web_name` reduces to one surname token; FPL breaks that assumption whenever it needs to disambiguate a name from someone else *league-wide* (not just within the club), producing `web_name`s like "Pedro Porro" (Tottenham's Pedro Porro Sauceda — a real, current first-teamer, not an academy edge case), "Fábio Vieira" (Arsenal), "Van de Ven" (Tottenham's Micky/Mickey van de Ven — whose first name also happens to be spelled differently by each provider, a second, independent miss reason on the same player), plus initial-prefixed forms ("B.Fernandes", "G.Jesus", "J.Cuenca"), hyphenated surnames ("Kesler-Hayden"), and "Jr"-suffixed forms ("Iling Jr"). Measured directly against the post-repair residual: of the 59 Premier League players who have a `team_id` but never resolved a football-data identity, **29 (roughly half) carry a multi-token `web_name` of this shape** — an honest, in-scope gap, not a bug: matching multi-word `web_name`s would mean token-set overlap or partial matching, which is a step toward the fuzzy-matching territory this fix deliberately stays out of.
- **Cross-league same-name pairs that are not merged, and should not be assumed to be the same person without more than a name to go on:** the full `players` table still contains a small number of normalised-name collisions between an FPL-only Premier League row and a football-data row for a *different* league's club (e.g. "Diego Coppola" — Paris FC (Ligue 1, football-data) vs Brighton & Hove Albion (Premier League, FPL); "Mamadou Sangare"/"Sangaré" — Racing Club de Lens (Ligue 1) vs Brentford (Premier League)). These are plausibly the same real person after a transfer the two providers haven't both caught up on, or may be two different people — this fix's candidate pool is deliberately scoped to Premier League clubs only (FPL is Premier-League-only data), so it never attempts, and never should attempt, a cross-league merge on name alone. A couple of purely football-data duplicate names also exist independent of FPL entirely (e.g. two different real players both named "Vitinha", one at PSG and one at Genoa) — a pre-existing identity ambiguity inherent to sharing a common name, unrelated to this fix.
- Squad and player pages must independently check for `team_id` presence and for `fpl_id`/`fd_id` presence, must not assume a name search returns exactly one Premier League row, and must never infer that two same-named rows in different leagues are the same person without a stronger signal than normalised name.
...
`news_items.published_at` is nullable (migration `0004_nullable_news_published_at.sql`, not yet applied against the live database — see the plan's Task 2 amendment): a feed's date is stored as `null`, never guessed at, when it is absent or unparseable. Phase B's news query must sort `ORDER BY published_at DESC NULLS LAST` explicitly — Postgres's default null ordering for a `DESC` sort is `NULLS FIRST`, which would put every undated story at the top of the feed.
