...
> **Review-fixes pass #3, Finding 2 (IMPORTANT):** `<LiveScores>` seeds its state with `useState(initial)`/`useRef(initial)`, which only run on first mount — invisible while zero fixtures are ever live (today), but on the first real matchday, `/scores` rendered `<LiveScores>` in a stable JSX position with no `key`, so a client-side navigation between two `?leagues=` URLs reconciled it as the *same* component instance and updated props in place instead of remounting: switching from a filter with live matches to a different filter that also has live matches kept the panel showing the **previous** filter's fixtures until the next poll, and even then merged into the stale ref rather than replacing it. Fixed two ways: Task 6's page now keys `<LiveScores>` on the selected league codes (`key={selected.join(',') || 'all'}`), forcing a clean remount — and therefore a fresh `useState`/`useRef` seeded from the newly server-rendered `initial` — on every filter change; and the mount effect here now polls once immediately (`start(); void poll();`) instead of waiting a full `POLL_MS`, so a user who just changed filters isn't stuck looking at (now-correct, but possibly stale-by-up-to-two-minutes) data. The immediate mount poll and `handleVisibilityChange`'s own immediate resume poll fire from disjoint events — mount vs. a later `visibilitychange` — so they cannot double-fire against each other.
...
>
> **Review-fixes pass #3, Finding 1 (IMPORTANT):** the version below still calls `getUpcoming(now, 20)` — unscoped by league, unlike `getLiveAndRecent(now)` right above it — then filters the resulting 20 rows down to the selected leagues in JavaScript, mirroring the `keep()` pattern this page already uses for `recent`. That is the same bug shape as Finding 1 above applied to a query with no `leagueIds` parameter at all: once combined near-term fixture volume across the five leagues exceeds 20, a league whose fixtures sort past the *unscoped* top 20 is truncated away before `keep()` ever sees them, and the page falsely renders "Nothing scheduled" for a league that has real fixtures in the database — invisible today (only 6 fixtures in the next 14 days, so nothing yet approaches 20), reproducible from the first or second matchday of the season. Fixed by giving `getUpcoming` an optional `leagueIds?: number[]` parameter mirroring `getFixturesInRange` exactly, including its empty-array-matches-nothing rule (`lib/site/queries/fixtures.ts`); the page resolves `selected` to ids once, up front (`selectedLeagueIds`, already computed here for `<LiveScores>`), and passes it to **both** `getLiveAndRecent` and `getUpcoming` so `limit` applies to an already-scoped result rather than to everything. **The limit must be applied after scoping, not before, because it is a page size on a per-league result set, not a global cutoff that happens to be filtered afterward** — reversing that order is exactly what let a real league's fixtures disappear behind an unrelated league's volume. The `keep()`/`shownRecent`/`shownUpcoming` JS filtering is removed entirely; `recent`/`upcoming` are used directly. See `.superpowers/sdd/task-6-report.md` for the RED test that failed against the unscoped implementation and the real `/scores`, `/scores?leagues=PD`, `/scores?leagues=XYZ` renders after the fix. The same page also now keys `<LiveScores>` on the selected league codes (Task 5, Finding 2) so a filter change forces a clean remount instead of reusing a stale instance.
...
> **Corrected in the review-fixes pass #3 (Finding 1).** The code above still shows `getUpcoming(now, 20)` unscoped plus JS-side `keep()`/`shownRecent`/`shownUpcoming` filtering — that is the version the bug was found in, not what ships. The current source resolves `selectedLeagueIds` once, up front, before the `Promise.all`, and passes it to both `getLiveAndRecent(now, selectedLeagueIds)` and `getUpcoming(now, 20, selectedLeagueIds)`; `recent`/`upcoming` are used directly with no further JS filtering; and `<LiveScores>` is given `key={selected.length > 0 ? selected.join(',') : 'all'}` (Task 5, Finding 2). See `app/scores/page.tsx` for the exact implementation — do not hand-copy the version above.
