...
```bash
node -e "for (const f of ['fd-matches-pl','fd-standings-pl','fd-team-57']) { const j = require('./tests/fixtures/'+f+'.json'); console.log(f, Object.keys(j).slice(0,4).join(',')); }"
```

Expected: keys including `matches`, `standings`, `squad` respectively.
